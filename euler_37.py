...
primes_file = 'euler_37_primes_to_million.txt'
primes = []
...
def getRotations( prime ):
    start = time.time()
    rotations = [str(prime)]
    rotations2 = [str(prime)]
    for i in range(1,len(rotations[-1])):
        rotations.append(rotations[-1][1:])
        rotations2.append(rotations2[-1][:-1])
    rotations.extend(rotations2)
